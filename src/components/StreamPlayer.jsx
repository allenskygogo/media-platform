...
export default function StreamPlayer({
  ...
  useEffect(() => {
    maxRef.current = savedSec
    lastTimeRef.current = savedSec
    lastSavedSecondRef.current = Math.floor(savedSec)
  }, [savedSec, videoUid])
  ...
  useEffect(() => {
    ...
    loadStreamSdk(() => {
      ...
      player.addEventListener('timeupdate', () => {
        ...
        if (isForced && !correctingSeekRef.current && ct > maxRef.current + 2) {
          correctingSeekRef.current = true
          player.currentTime = maxRef.current
          setTimeout(() => { correctingSeekRef.current = false }, 250)
          return
        }
        if (ct >= lastTimeRef.current || ct <= maxRef.current + 1) {
      ...
      if (isForced) {
        const clampForwardSeek = () => {
          if (correctingSeekRef.current || doneRef.current) return
          if ((player.currentTime || 0) > maxRef.current + 2) {
            correctingSeekRef.current = true
            player.currentTime = maxRef.current
            lastTimeRef.current = maxRef.current
            setTimeout(() => { correctingSeekRef.current = false }, 250)
          }
        }
  ...
  useEffect(() => {
    // In forced mode: block shortcuts that can seek forward or trigger the iframe fullscreen.
    // Space is intentionally allowed so first-time viewers can pause/resume.
    const FORCED_BLOCK  = ['ArrowRight', 'ArrowUp', 'f', 'F', 'm', 'M']
