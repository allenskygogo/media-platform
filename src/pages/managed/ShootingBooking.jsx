...
const TIME_SLOTS = [
  { key: 'morning',   label: '上午場',  time: '09:00 – 12:00', icon: '🌅' },
  { key: 'afternoon', label: '下午場',  time: '13:00 – 17:00', icon: '☀️' },
  { key: 'evening',   label: '晚上場',  time: '18:00 – 21:00', icon: '🌙' },
]
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
          ...
          {step >= 2 && (
            <div className="card">
              ...
              <div className="card-body">
                <p style={{ fontSize: 14, color: 'var(--gray-600)', marginBottom: 16 }}>拍攝日期：<strong>{date}</strong></p>
                <div className="time-slots">
                  {TIME_SLOTS.map(ts => (
                    <button key={ts.key} className={`time-slot-btn ${slot === ts.key ? 'selected' : ''}`}
                      ...
                      <div className="time-slot-info"><strong>{ts.label}</strong><span>{ts.time}</span></div>
                    </button>
                  ))}
                </div>
              </div>
