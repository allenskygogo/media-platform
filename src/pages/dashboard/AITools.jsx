    ...
    try {
      const payload = {
        ...
        instruction: nextScriptType === 'knowledge'
          ...
          : nextScriptType === 'opinion'
            ? '請把已上傳的知識庫 PDF 當成產生腳本的主要依據，不只是練習判斷用。若知識庫有說觀點案例或句式，必須優先貼近該句式。語氣要非常白話，把使用者當成完全不懂的新手，句子短、意思直、不要學術、不要難懂，國中生也要看得懂。輸出順序必須只有四段：開篇、歸納論據或正反論證、總結觀點、金句。開篇一定要用否定句或反問句，將原選題改成有立場的觀點，例如「公寓與住宅的購買解析」改成「千萬不要買公寓」。論據全部寫在「歸納論據或正反論證」同一段裡，不要拆成兩個欄位；段內要清楚標明每段文法，例如「論據1（不同視角）」「論據2（權衡利弊）」「論據3（倒打一耙）」或「正面論證（維度提升）」「反面論證（負面後果）」。每一個文法都要是一整段，不可以只有一句話；每段至少 3 到 5 句，要有生活場景、白話比喻或觀眾日常會遇到的情境。觀點不是教育粉絲，也不是罵觀眾；觀點是分享我覺得這件事對或錯，並站在我的受眾那一邊。內容要讓特定受眾感覺：你站在我這邊、你把我不敢講的話講出來了、你替我出了一口氣、我想支持你、我想轉發給某個人看。禁止輸出只有提綱式的一句論據，禁止輸出論據1/論據2作為獨立段落，也不要輸出通用的反常識立場/錯誤認知/核心觀點/例子對比/行動收束框架。'
