...
- 當 input.scriptType = "story" 時，必須嚴格使用說故事腳本 SOP：
  ...
  最後一個轉機必須寫得最多、最爽、最解氣，要有爽劇反轉感。可以讓前面欺壓、看不起、打壓主角的人，最後回頭來投靠、拜託、求合作、認錯或被現實打臉。說故事要把經典橋段寫成爽劇，不只是平淡解決問題。
...
const STORY_EVENT_RULE = '當 scriptType=story 或腳本類型=說故事時，可用困境橋段：盲目自信、家門不幸、天災人禍、惡霸欺壓、過河拆橋、至親背叛、強權干涉。可用轉機橋段：通關密碼、至親所託、天賦異稟、意外之喜、因禍得福、貴人相助、破釜沈舟。這些都是可穿插使用的故事文法，請依故事邏輯選最適合的組合，不要固定只用同一組。每次生成說故事腳本時，困難與轉機橋段必須從清單中隨機或輪替變換。除非題目明確需要，禁止每次固定使用「盲目自信、通關密碼、過河拆橋、貴人相助」這組範例橋段。至親所託的意思是親人對主角的寄望、託付或希望，例如父母、伴侶、孩子、長輩把某個期待交到主角身上；如果是朋友、同事、客戶、老師或外人幫助主角，那是貴人相助，不是至親所託。最後一個轉機必須寫得最多、最爽、最解氣，要有爽劇反轉感。可以讓前面欺壓、看不起、打壓主角的人，最後回頭來投靠、拜託、求合作、認錯或被現實打臉。說故事要把經典橋段寫成爽劇，不只是平淡解決問題。【困境與轉機】的 body 必須只用固定格式：困難：橋段名 換行 內容；空一行；轉機：橋段名 換行 內容；空一行；困難：橋段名 換行 內容；空一行；轉機：橋段名 換行 內容。禁止寫成一整段，禁止使用「困難1（橋段名）：」或「轉機1（橋段名）：」。橋段名稱只是標籤，內容才是正文。不要把「盲目自信、過河拆橋、貴人相助」寫成一句成語解釋，也不要順接成語；要把它翻成一整段事件。例如盲目自信不是寫「我太自信所以失敗」，而是寫：公司虧錢、宣傳片案子變少、新聞看到短視頻崛起、我以為十年導演背景做短視頻很簡單、接了客戶後播放量只有幾百、客戶打電話要退款、帳上沒錢只好借錢補洞。整段事件要讓觀眾自然感覺出「盲目自信」。'

function ensureStoryEventRule(agent) {
  if (!agent || agent.feature_key !== 'script') return agent
  const withRule = text => String(text || '').includes('整段事件要讓觀眾自然感覺出「盲目自信」')
    ? text
...
async function getAIAgent(feature, env) {
  ...
  if (!env.SUPABASE_URL || !key) return ensureStoryEventRule(fallback)
  ...
  try {
    ...
    const data = await response.json().catch(() => [])
    if (!response.ok) return ensureStoryEventRule(fallback)
    return ensureStoryEventRule(Array.isArray(data) && data[0] ? data[0] : fallback)
  } catch (_) {
    return ensureStoryEventRule(fallback)
  }
}
