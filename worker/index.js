...
const SCRIPT_TOPIC_GENERATION_RULE = '當 input.task = "generate_topics" 時，必須把已上傳的腳本知識庫 PDF 當成選題生成的主要依據，不只是後面寫腳本才使用。若有 file_search / vector store，必須優先依據 TOP LEVEL TRAFFIC 知識庫中的案例、句式、腳本 SOP、選題維度與限制條件來產生選題，禁止只用通用短影音建議。輸出必須是 JSON array，剛好 8 筆，每筆格式為 {"element":"...","text":"...","traffic":"high|medium|low"}。element 是前端顯示的爆款元素標籤，絕對不可省略、不可改名成 tag/category/label，也不可放空。除非 input.scriptType="process"，否則 8 筆 element 必須依序固定為：奇葩、人群、懷舊、最差、頭牌、荷爾蒙、反差、成本。如果 input.scriptType="process"，8 筆 element 必須依序固定為：過程展示、測評產品、任務挑戰、事件體驗、過程展示、測評產品、任務挑戰、事件體驗。如果 input.scriptType="knowledge"，選題要能接出教知識三段式 SOP：場景難題、低行動成本解決方案、具體操作過程；每題都要有具體受眾、場景、痛點或錯誤行為，並符合信息多、效果快、料夠猛。'
const TOPICS_KNOWLEDGE_RULE = '產生爆款選題時，若有 file_search / vector store 或已上傳知識庫 PDF，必須優先依據 TOP LEVEL TRAFFIC 知識庫的案例、句式、腳本 SOP、選題維度與限制條件，不可只用通用短影音建議。每一筆選題都必須有 element 欄位，這是前端的爆款元素標籤，絕對不可省略。非曬過程選題的 element 必須使用並依序輪替：奇葩、人群、懷舊、最差、頭牌、荷爾蒙、反差、成本；曬過程選題只使用：過程展示、測評產品、任務挑戰、事件體驗。當 input.scriptType="knowledge" 或腳本類型=教知識時，選題必須能接出教知識三段式 SOP：場景難題、低行動成本解決方案、具體操作過程；每題要具體指出受眾、真實場景、痛點或錯誤行為，並符合信息多、效果快、料夠猛。'
